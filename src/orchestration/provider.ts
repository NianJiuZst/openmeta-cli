...
function parseProvider(value: string | undefined): LLMProvider {
	...
	if (
		!['openai', 'minimax', 'moonshot', 'zhipu', 'gemini', 'claude', 'custom'].includes(provider)
	) {
		throw new Error(
			'provider must be "openai", "minimax", "moonshot", "zhipu", "gemini", "claude", or "custom".',
		);
	}
...
export class ProviderOrchestrator {
	async list(): Promise<void> {
		...
		ui.hero({
			...
			title:
				names.length > 0
					? 'Saved provider profiles are ready to switch'
					: 'No provider profiles saved yet',
			subtitle:
				'Provider profiles let you keep multiple LLM backends available without repeating config set commands.',
	...
	async use(nameInput: string, options: ProviderUseOptions = {}): Promise<void> {
		...
		if (!profile) {
			throw new Error(
				`Provider profile "${name}" does not exist. Run "openmeta provider list" to see saved profiles.`,
			);
		}
