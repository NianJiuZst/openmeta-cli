...
export class ConfigOrchestrator {
	async view(): Promise<void> {
		...
		ui.keyValues('LLM', [
			...
			{
				label: 'Streaming',
				value: config.llm.stream ? 'yes' : 'no',
				tone: config.llm.stream ? 'info' : 'muted',
			},
			{
				label: 'Extra headers',
				value:
		...
		ui.keyValues('Scoring weights', [
			...
			{ label: 'Freshness', value: `${(sw.freshness * 100).toFixed(0)}%`, tone: 'info' },
			{
				...
				value: `${(sw.onboardingClarity * 100).toFixed(0)}%`,
				tone: 'info',
			},
			...
			{ label: 'Technical Match', value: `${(ow.technicalMatch * 100).toFixed(0)}%`, tone: 'info' },
			{
				...
				value: `${(ow.opportunityScore * 100).toFixed(0)}%`,
				tone: 'info',
			},
		]);
	...
	async set(key: string, value: string): Promise<void> {
		...
		if (key === 'userProfile.techStack') {
			...
			if (
				!['openai', 'minimax', 'moonshot', 'zhipu', 'gemini', 'claude', 'custom'].includes(value)
			) {
				throw new Error(
			...
			updated = await configService.update({
				llm: { ...config.llm, reasoningEffort: parseLLMReasoningEffort(value) },
			});
		} else if (key === 'llm.stream') {
			updated = await configService.update({
				llm: { ...config.llm, stream: this.parseBoolean(value, key) },
			});
			...
			const newWeights = normalizeOverallWeights({
			...
			updated = await configService.update({
				scoring: {
					...
					overallWeights: preset.overallWeights,
					preset: preset.name,
				},
			});
	...
	async scoring(): Promise<void> {
		...
		ui.hero({
			...
			subtitle:
				'Choose a preset or customize individual weights. Higher weight = more influence on the final ranking.',
			tone: 'accent',
		});
		...
		if (selectedPreset !== 'custom') {
			...
			const entries: Array<{ key: keyof ScoringWeights; label: string; default: number }> = [
				...
				{
					key: 'impact',
					label: 'Impact (repo stars and visibility)',
					default: Math.round(current.impact * 100),
				},
				{
					key: 'riskPenalty',
			...
			ui.section(
				'Overall weights',
				'Adjust the balance between technical match and opportunity score.',
			);
		...
		ui.keyValues('Preview — Scoring weights', [
			...
			{
				label: 'Onboarding Clarity',
				value: `${(newWeights.onboardingClarity * 100).toFixed(0)}%`,
				tone: 'info',
			},
			{
				label: 'Merge Potential',
				value: `${(newWeights.mergePotential * 100).toFixed(0)}%`,
				tone: 'info',
			},
			{ label: 'Impact', value: `${(newWeights.impact * 100).toFixed(0)}%`, tone: 'info' },
			{
				label: 'Risk Penalty',
				value: `${(newWeights.riskPenalty * 100).toFixed(0)}%`,
				tone: 'info',
			},
			{
				label: 'Technical Match',
				value: `${(newOverallWeights.technicalMatch * 100).toFixed(0)}%`,
				tone: 'info',
			},
			{
				label: 'Opportunity Score',
				value: `${(newOverallWeights.opportunityScore * 100).toFixed(0)}%`,
				tone: 'info',
			},
		]);
