...
export class AgentOrchestrator {
	...
	async run(options: AgentRunOptions = {}): Promise<void> {
		...
		const runChecks =
			typeof options.runChecks === "boolean" ? options.runChecks : !headless;
		...
		this.renderOpportunityList(
			...
			rankedIssues.slice(0, 5),
		);
		const selectedIssue = headless
			? issueRankingService.selectIssueForAutomation(
					rankedIssues,
					config.automation.minMatchScore,
				)
		...
		this.renderAgentStage(
			"prepare",
			completedStages,
			`Cloning and inspecting ${selectedIssue.repoFullName}.`,
		);
		...
		const patchDraftResult = await ui.task(
			...
			async () =>
				llmService.generatePatchDraft(selectedIssue, workspace, memory),
		);
		...
		const implementationWorkspace = this.buildImplementationWorkspace(
			workspace,
			patchDraft,
		);
		...
		this.renderAgentStage(
			"pr",
			completedStages,
			"Drafting PR narrative and deciding whether to open a real draft PR.",
		);
		...
		const patchDraftMarkdown =
			contentService.formatPatchDraftMarkdown(patchDraft);
		const prDraftMarkdown =
			contentService.formatPullRequestDraftMarkdown(prDraft);

		const contributionPullRequest =
			await this.submitContributionPullRequestIfPossible({
				config,
				allowRealPr:
					patchDraftResult.status === "success" &&
		...
		const finalProofMarkdown = proofOfWorkService.renderMarkdown(
	...
	async scout(options: ScoutRunOptions | number = {}): Promise<void> {
		...
		const refresh =
			typeof options === "number" ? false : Boolean(options.refresh);
		const localOnly =
			typeof options === "number" ? false : Boolean(options.localOnly);
		const config = await configService.get();
		...
		ui.hero({
			...
			title: localOnly
				? "Read the field while the model stays offline"
				: "Read the field before you spend your focus",
			subtitle: localOnly
		...
		const rankedIssues = await ui.task(
			...
			async () =>
				issueRankingService.loadRankedIssues(config, { refresh, localOnly }),
		);
		if (rankedIssues.length === 0) {
			ui.emptyState(
				"OpenMeta Scout",
		...
		ui.stats("Scout snapshot", [
			{
				label: "Ranked issues",
				value: String(rankedIssues.length),
				tone: "success",
			},
			{
				label: "Showing",
				value: String(Math.min(limit, rankedIssues.length)),
				tone: "info",
			},
			{
				label: "Top score",
				value: String(rankedIssues[0]?.opportunity.overallScore || 0),
				tone: "accent",
			},
			{
				label: "Profile threshold",
				value: `${config.automation.minMatchScore}`,
				tone: "info",
			},
		]);
		this.renderOpportunityList(
			"Ranked opportunities",
			issueRankingService.diversifyScoutIssues(rankedIssues, limit),
		);
	}

	async showInbox(): Promise<void> {
		const items = [...inboxService.load().items].sort((left, right) =>
		...
		ui.hero({
			...
			title:
				items.length > 0
					? "Keep the right opportunities within arm's reach"
					: "The shortlist is quiet for now",
			subtitle:
				"This is the retained shortlist: drafted, scored, and kept close for the next sharp move.",
		...
		ui.stats("Inbox snapshot", [
			...
			{
				label: "Top score",
				value: String(items[0]?.overallScore || 0),
				tone: "info",
			},
			{
				label: "Latest",
				value: this.formatDate(items[0]?.generatedAt),
				tone: "info",
			},
		]);
	...
	async showProofOfWork(): Promise<void> {
		...
		ui.hero({
			...
			title:
				records.length > 0
					? "Keep a readable trail of work that actually landed"
					: "No trail has been written yet",
			subtitle:
				"Every run leaves a ledger of what was chosen, what changed, and what reached publication.",
		...
		if (records.length === 0) {
			ui.emptyState(
				...
				'Run "openmeta agent" to generate contribution evidence.',
			);
			return;
		}
		...
		ui.stats("Proof of work snapshot", [
			...
			{
				label: "Published",
				value: String(records.filter((record) => record.published).length),
				tone: "accent",
			},
			{
				label: "Top score",
				value: String(records[0]?.overallScore || 0),
				tone: "info",
			},
			{
				label: "Latest",
				value: this.formatDate(records[0]?.generatedAt),
				tone: "info",
			},
		]);
	...
	private renderAgentStage(
		...
		const currentIndex = AGENT_STAGES.findIndex(
	...
	private showSelectedOpportunity(issue: RankedIssue, headless: boolean): void {
		ui.card({
			label: "OpenMeta Agent",
			title: headless
				? "Automation selected this opportunity"
		...
		ui.stats("Selected issue metrics", [
			{
				label: "Overall",
				value: String(issue.opportunity.overallScore),
				tone: "success",
			},
			{ label: "Match", value: String(issue.matchScore), tone: "info" },
			{
				label: "Opportunity",
				value: String(issue.opportunity.score),
				tone: "accent",
			},
		...
		ui.keyValues("Selected issue context", [
			{
				label: "Labels",
				value: issue.labels.join(", ") || "none",
				tone: "info",
			},
			{
				label: "Tech requirements",
				value: issue.analysis.techRequirements.join(", ") || "n/a",
				tone: "info",
			},
			{
				label: "Workload",
				value: issue.analysis.estimatedWorkload || "n/a",
				tone: "info",
			},
			{
				label: "Updated",
				value: this.formatDate(issue.updatedAt),
				tone: "info",
			},
		]);
	}

	private showWorkspaceSummary(
		workspace: RepoWorkspaceContext,
		memory: ContributionAgentResult["memory"],
	): void {
		ui.stats("Workspace snapshot", [
			{
				label: "Candidate files",
				value: String(workspace.candidateFiles.length),
				tone: "success",
			},
			{
			...
			{
				label: "Memory dossiers",
				value: String(memory.generatedDossiers),
				tone: "info",
			},
		]);
		...
		ui.keyValues("Workspace details", [
			...
			{ label: "Default branch", value: workspace.defaultBranch, tone: "info" },
			{
				...
				value: workspace.branchName || "workspace already dirty",
				tone: "info",
			},
			{
				label: "Top-level files",
				value: workspace.topLevelFiles.slice(0, 8).join(", ") || "n/a",
				tone: "info",
			},
		]);
				...
				lines:
					workspace.testCommands.length > 0
						? workspace.testCommands
								...
								.map(
									(command) =>
										`${command.command} (${command.reason}; ${command.source})`,
								)
	...
	private showValidationSummary(
		...
		ui.stats("Validation snapshot", [
			...
			},
			{
				...
				tone: counts.passed > 0 ? "success" : "muted",
			},
			{
				label: "Failed",
				value: String(counts.failed),
				tone: counts.failed > 0 ? "warning" : "muted",
			},
			{
				label: "Unavailable",
				value: String(counts.unavailable),
				tone: counts.unavailable > 0 ? "warning" : "muted",
			},
		]);
	...
	private showArtifactPreview(input: {
		...
		ui.card({
			...
			subtitle:
				"OpenMeta generated a dossier, patch draft, PR draft, inbox update, and proof-of-work update.",
			lines: [
		...
		ui.keyValues("Artifact details", [
			{
				label: "Issue",
				value: `${input.issue.repoFullName}#${input.issue.number}`,
				tone: "info",
			},
			{
				label: "Overall score",
				value: String(input.issue.opportunity.overallScore),
				tone: "success",
			},
			{
				label: "Changed files",
			...
			{
				label: "Validation",
				value: this.formatValidationSummary(input.validationResults),
				tone: "info",
			},
			{
				label: "Artifact branch",
				value: ARTIFACT_PUBLISH_BRANCH,
				tone: "info",
			},
		]);
	}
	...
	private async validateConfig(
		...
		if (!config.github.pat || !config.github.username) {
			throw new Error(
				'GitHub configuration is incomplete. Please run "openmeta init" first.',
			);
		}

		if (requireLlm && !config.llm.apiKey) {
			throw new Error(
				'LLM API configuration is incomplete. Please run "openmeta init" first.',
			);
		}
	}
	...
	private async initializeClients(
		...
		if (!ghValid) {
			throw new Error(
				'GitHub credentials validation failed. Run "openmeta init" to refresh your token.',
			);
		}
		...
		const llmValid = await ui.task(
			...
			async () => {
				...
				if (!valid) {
					...
					throw new Error(
						`LLM validation failed${detail ? `: ${detail}` : ""}`,
					);
				}
				return true;
			},
		...
		if (!llmValid) {
			throw new Error(
				'LLM API connection failed. Run "openmeta init" to update your provider settings.',
			);
		}
	}
	...
	private async promptForIssue(issues: RankedIssue[]): Promise<RankedIssue> {
		...
		try {
			...
			logger.warn(
				"Interactive select UI is unavailable. Falling back to numeric input.",
			);
		}
	...
	private prepareLocalArtifactPaths(issue: RankedIssue) {
		...
		const artifactDir = ensureDirectory(
			join(getOpenMetaArtifactRoot(), getLocalDateStamp(), dirName),
		);
	...
	private writeLocalArtifacts(input: {
		...
		writeFileSync(
			input.artifacts.patchDraftPath,
			input.patchDraftMarkdown,
			"utf-8",
		);
		...
		writeFileSync(
			input.artifacts.proofOfWorkPath,
			input.proofMarkdown,
			"utf-8",
		);
	}
	...
	private buildImplementationWorkspace(
		...
		const existingSnippetPaths = new Set(
			workspace.snippets.map((snippet) => snippet.path),
		);
		const missingSnippetPaths = patchPaths.filter(
			(path) => !existingSnippetPaths.has(path),
		);
		...
		if (
			...
			patchPaths.every((path) => workspace.candidateFiles.includes(path))
		) {
			return workspace;
		}
		...
		if (extraSnippets.length > 0) {
			logger.info(
				`Loaded ${extraSnippets.length} patch target file(s) into implementation context.`,
			);
		}
	...
	private mergeSnippets(
		current: RepoFileSnippet[],
		next: RepoFileSnippet[],
	): RepoFileSnippet[] {
	...
	private async generateConcretePatch(
		...
		if (draftOnly) {
			...
			logger.info(
				"Skipping generated file edits because draft-only mode is active.",
			);
			return {
				changedFiles: [],
		...
		if (workspace.workspaceDirty) {
			...
			logger.warn(
				`Skipping generated file edits because the workspace is dirty: ${workspace.workspacePath}`,
			);
			return {
				changedFiles: [],
				validationResults: workspace.testResults,
		...
		try {
			const implementation = await ui.task(
				...
				async () =>
					llmService.generateImplementationDraft(issue, workspace, patchDraft),
			);
			if (implementation.status !== "success") {
				...
				logger.warn(
					"Skipping automatic file edits because the implementation draft requires review.",
				);
				return {
					changedFiles: [],
					validationResults: workspace.testResults,
			...
			if (implementation.data.fileChanges.length === 0) {
				ui.callout({
					...
					lines: [
						"Draft artifacts will still be generated so you can continue from the research output.",
					],
					tone: "warning",
				});
				logger.warn(
			...
			const changedFiles = await ui.task(
				...
				async () =>
					workspaceService.applyGeneratedChanges(
			...
			if (changedFiles.reviewRequired) {
				this.showStructuredReviewNotice({
					...
					lines: [
						changedFiles.reviewReason ||
							"Review the generated patch before applying it manually.",
					],
				});
				logger.warn(
					`Generated patch requires review: ${changedFiles.reviewReason || "unspecified reason"}`,
				);
				return {
			...
			if (changedFiles.appliedFiles.length === 0) {
				ui.callout({
					...
					subtitle:
						"The proposed patch matched the current workspace or resolved to no effective write.",
					lines: [
						"Draft artifacts will still be preserved for manual follow-up.",
						...changedFiles.skippedFiles
							.slice(0, 3)
			...
			logger.success(
				`Applied ${changedFiles.appliedFiles.length} workspace file updates`,
			);
			...
			if (
				...
				this.hasBlockingValidationFailures(validationResults)
			) {
	...
	private async publishArtifactsIfNeeded(input: {
		...
		const shouldCommit = input.headless
			? true
			: await this.promptForCommitConfirmation();
		if (!shouldCommit) {
			return { published: false };
		}
		...
		if (!gitInitialized) {
			throw new Error(
				`Failed to initialize the target repository at ${targetRepo.path}.`,
			);
		}

		const commitMessage = `feat(agent): draft contribution for ${input.issue.repoFullName}#${input.issue.number}`;
		const finalConfirm = input.headless
			? true
			: await this.promptForFinalCommitConfirmation(commitMessage);
		if (!finalConfirm) {
			return { published: false };
		}
		...
		const publishResult = await gitService.writeAndPublish(
			[
				{
					path: join(artifactRelativeDir, "dossier.md"),
					content: input.dossier,
				},
				{
					path: join(artifactRelativeDir, "patch-draft.md"),
					content: input.patchDraftMarkdown,
				},
				{
					path: join(artifactRelativeDir, "pr-draft.md"),
					content: input.prDraftMarkdown,
				},
				{
					path: join(
						"memory",
						`${input.issue.repoFullName.replace(/\//g, "__")}.md`,
					),
					content: input.memoryMarkdown,
				},
		...
		if (!publishResult) {
			throw new Error(
				"OpenMeta could not publish the generated contribution artifacts.",
			);
		}
	...
	private async submitContributionPullRequestIfPossible(input: {
		...
		if (!input.allowRealPr) {
			logger.warn(
				"Skipping real draft PR creation because one or more structured drafts require review.",
			);
			return {
		...
		const hasValidationFailures = input.validationResults.some(
			(result) => !result.passed,
		);
		const hasBlockingValidationFailures = this.hasBlockingValidationFailures(
			input.validationResults,
		);
		if (input.headless && hasBlockingValidationFailures) {
			logger.warn(
				"Skipping real draft PR creation because validation failed in headless mode.",
			);
			return {
				changedFiles: input.changedFiles,
				validationResults: input.validationResults,
		...
		if (!input.headless) {
			...
			const shouldCreatePr = await this.promptForContributionPrConfirmation(
				input.issue,
			);
			if (!shouldCreatePr) {
				return {
			...
			if (hasValidationFailures) {
				const continueWithFailures =
					await this.promptForFailedValidationConfirmation();
		...
		try {
			const contributionPullRequest =
				await contributionPrService.submitDraftPullRequest({
					issue: input.issue,
			...
			ui.card({
				...
				subtitle:
					"The generated patch has been pushed to your fork and turned into a real upstream draft PR.",
				lines: [
					`Repository: ${input.issue.repoFullName}`,
		...
		ui.keyValues("Headless run impact", [
			{
				...
				value: "skipped after this confirmation",
				tone: "warning",
			},
			{
				label: "Artifact publication",
				value: "may commit and push automatically",
				tone: "warning",
			},
			{
				label: "Draft PR creation",
				value: "may open a real upstream draft PR",
				tone: "warning",
			},
			{
				label: "Disable automation",
				value: "openmeta automation disable",
				tone: "info",
			},
		]);
	...
	private async promptForFinalCommitConfirmation(
	...
	private async promptForContributionPrConfirmation(
		issue: RankedIssue,
	): Promise<boolean> {
	...
	private async promptForFailedValidationConfirmation(): Promise<boolean> {
		const { continueWithFailures } = await prompt<{
			continueWithFailures: boolean;
		}>([
			{
				type: "confirm",
				name: "continueWithFailures",
				message:
					"Some validation commands failed. Continue and open a draft PR anyway?",
				default: false,
			},
		]);
	...
	private hasBlockingValidationFailures(results: TestResult[]): boolean {
		return results.some(
			(result) =>
				!result.passed && !this.isInfrastructureValidationFailure(result),
		);
	}
	...
	private async attemptValidationRepair(input: {
		...
		const currentFiles = workspaceService.readWorkspaceFiles(
		...
		if (repairDraft.status !== "success") {
			this.showStructuredReviewNotice({
				...
				subtitle:
					"OpenMeta marked the repair patch as review-required, so it will not be applied automatically.",
				lines: [
			...
			logger.warn(
				"Skipping validation repair because the generated patch requires review.",
			);
			return {
				changedFiles: input.changedFiles,
				validationResults: input.validationResults,
		...
		if (repairDraft.data.fileChanges.length === 0) {
			logger.warn(
				"Validation repair pass did not produce any additional safe file edits.",
			);
			return null;
		}
		...
		if (repairedFiles.reviewRequired) {
			this.showStructuredReviewNotice({
				...
				lines: [
					repairedFiles.reviewReason ||
						"Review the generated repair before applying it manually.",
				],
			});
			logger.warn(
				`Validation repair requires review: ${repairedFiles.reviewReason || "unspecified reason"}`,
			);
			return {
				changedFiles: input.changedFiles,
				validationResults: input.validationResults,
		...
		return {
			changedFiles: [
				...new Set([...input.changedFiles, ...repairedFiles.appliedFiles]),
			],
			validationResults,
			reviewRequired: false,
		};
	...
	private countValidationStates(results: TestResult[]): {
		passed: number;
		failed: number;
		unavailable: number;
	} {
	...
	private showResult(result: ContributionAgentResult): void {
		ui.hero({
			...
			subtitle:
				"OpenMeta moved from issue signal to working artifacts and left the full trail in a readable state.",
			lines: [
		...
		ui.stats("Run summary", [
			{
				label: "Overall score",
				value: String(result.issue.opportunity.overallScore),
				tone: "success",
			},
			{
				label: "Match score",
				value: String(result.issue.matchScore),
				tone: "info",
			},
			{
				label: "Changed files",
		...
		ui.keyValues("Run details", [
			{
				label: "Issue",
				value: `${result.issue.repoFullName}#${result.issue.number}`,
				tone: "info",
			},
			{
				label: "Workspace",
				value: result.workspace.workspacePath,
				tone: "info",
			},
			{
				label: "Branch",
				value: result.workspace.branchName || "workspace already dirty",
				tone: "info",
			},
			{
				label: "Changed files",
	...
	private async ensureTargetRepo(
		config: AppConfig,
	): Promise<TargetRepoContext> {
		if (config.github.targetRepoPath) {
			if (!existsSync(config.github.targetRepoPath)) {
				throw new Error(
					`Configured target repository path does not exist: ${config.github.targetRepoPath}`,
				);
			}
			...
			const repoInfo = await this.getGitHubRepositoryInfo(
				parsedRepo.owner,
				parsedRepo.repo,
			);
		...
		const remoteRepo = await this.ensureManagedRemoteRepo(
			config.github.username,
			repoName,
		);
		await this.ensureOriginRemote(git, remoteRepo.cloneUrl);
		await this.prepareLocalRepository(
			git,
			remoteRepo.defaultBranch,
			remoteRepo.hasCommits,
		);
	...
	private async ensureOriginRemote(
		git: SimpleGit,
		remoteUrl: string,
	): Promise<void> {
		...
		if (existingUrl && existingUrl !== remoteUrl) {
			logger.warn(
				`Origin remote already points to ${existingUrl}. Leaving the existing remote untouched.`,
			);
		}
	}
	...
	private parseGitHubRepository(remoteUrl: string): {
		owner: string;
		repo: string;
	} {
