...
export class AgentOrchestrator {
	...
	async run(options: AgentRunOptions = {}): Promise<void> {
		...
		const issueTarget = options.issue
			? resolveGitHubIssueTarget(options.issue, options.repo)
			: undefined;
		const repoFullName =
			issueTarget?.repoFullName ??
			(options.repo ? parseGitHubRepoFullName(options.repo) : undefined);
		...
		if (!selectedIssue) {
			ui.emptyState(
				'OpenMeta Agent',
				issueTarget
					? 'Target issue could not be selected'
	...
	async scout(options: ScoutRunOptions | number = {}): Promise<void> {
		...
		const repoFullName =
			typeof options === 'number' || !options.repo
				? undefined
				: parseGitHubRepoFullName(options.repo);
	...
	private async publishArtifactsIfNeeded(input: {
		...
		const publishResult = await gitService.writeAndPublish(
			[
				...
				{
					path: join('memory', `${input.issue.repoFullName.replace(/\//g, '__')}.md`),
	...
	private async ensureTargetRepo(config: AppConfig): Promise<TargetRepoContext> {
		if (config.github.targetRepoPath) {
			...
			const defaultBranch = await this.resolveConfiguredTargetDefaultBranch(
				git,
				parsedRepo.owner,
				parsedRepo.repo,
			);
	...
	private async resolveConfiguredTargetDefaultBranch(
		git: SimpleGit,
		owner: string,
		repo: string,
	): Promise<string> {
