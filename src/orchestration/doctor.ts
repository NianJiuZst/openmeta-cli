...
export class DoctorOrchestrator {
  ...
  private checkLlmConfig(config: AppConfig): DoctorCheck {
    ...
    return {
      ...
      detail: `${config.llm.modelName} at ${config.llm.apiBaseUrl}; reasoning ${config.llm.reasoningEffort || 'none'}; streaming ${config.llm.stream ? 'yes' : 'no'}; interaction output ${config.llm.showInteraction ? 'yes' : 'no'}; key ${ui.maskSecret(config.llm.apiKey)}`,
    };
  }
