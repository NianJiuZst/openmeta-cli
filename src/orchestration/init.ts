...
export class InitOrchestrator {
	async execute(): Promise<void> {
				...
				while (!ghValid) {
					...
					if (!ghValid) {
						this.renderStep(
							"github",
							completedSteps,
							"GitHub credentials need to be retried.",
							true,
						);
						ui.callout({
							label: "OpenMeta Init",
							...
							subtitle:
								"OpenMeta could not verify repository access with the token and username you entered.",
							lines: [
						...
						if (!retry) {
							ui.callout({
								...
								title: "Setup paused",
								subtitle:
									'GitHub access was not configured. Run "openmeta init" again whenever you are ready.',
								tone: "warning",
							});
							return false;
						}
		...
		await stepOrSkip(
			...
			() => {
				...
				ui.keyValues("LLM provider connected", [
					{
						label: "Provider",
						value: selectedProvider?.name ?? providerValue,
						tone: "success",
					},
				...
				while (!llmValid) {
					providerValue = (await selectPrompt<string>({
						...
						choices: LLM_PROVIDER_PRESETS.map((provider) => ({
							...
							description:
								provider.baseUrl || "Bring your own compatible endpoint",
						})),
					})) as AppConfig["llm"]["provider"];

					selectedProvider = findLLMProviderPreset(
						providerValue as AppConfig["llm"]["provider"],
					);
					if (!selectedProvider)
					...
					if (!llmValid) {
						...
						this.renderStep(
							"llm",
							completedSteps,
							"Provider validation needs to be retried.",
							true,
						);
						ui.callout({
							label: "OpenMeta Init",
							...
							subtitle:
								"OpenMeta could not connect to the configured provider with the selected model and API key.",
							lines: [
								"Check provider endpoint, model name, and API key.",
								"If you use a proxy or compatible endpoint, confirm the base URL is correct.",
								...(validationDetail
									? [`Provider detail: ${validationDetail}`]
									: []),
							],
							tone: "warning",
						});
						...
						if (!retry) {
							ui.callout({
								...
								title: "Setup paused",
								subtitle:
									'The LLM provider was not configured. Run "openmeta init" again when you want to continue.',
								tone: "warning",
							});
							return false;
						}
				...
				({ techStack } = await prompt<{ techStack: string[] }>([
					{
						...
						name: "techStack",
						message:
							"  Select your tech stack (Space to select, Enter to confirm):",
						...
						validate: (input: string[]) =>
							input.length > 0 || "Select at least one technology",
					},
				]));
					...
					choices: [
						{
							name: "Beginner",
							value: "beginner",
							description: "New to the stack, prefer guided issues.",
						},
						{
							name: "Intermediate",
							value: "intermediate",
							description:
								"Comfortable with the stack, can handle moderate tasks.",
						},
						{
							name: "Advanced",
							value: "advanced",
							description: "Deep experience, ready for complex changes.",
						},
					],
				});
				...
				({ focusAreas } = await prompt<{ focusAreas: string[] }>([
					{
						...
						name: "focusAreas",
						message:
							"  Select your focus areas (Space to select, Enter to confirm):",
						choices: FOCUS_AREA_CHOICES.map((area) => ({
							...area,
							checked: config.userProfile.focusAreas.includes(area.value),
						})),
						validate: (input: string[]) =>
							input.length > 0 || "Select at least one focus area",
					},
				]));
			...
			const scheduleResponse = await prompt<{ scheduleTime: string }>([
				{
					...
					filter: (input: string) => input.trim(),
					validate: (input: string) =>
						/^([01]\d|2[0-3]):([0-5]\d)$/.test(input) || "Enter time as HH:mm.",
				},
			]);
				...
				choices: [
					{
						name: "Research Notes",
						value: "research_note",
						description: "Safer default for unattended runs.",
					},
					{
						name: "Development Diary",
						value: "development_diary",
						description:
							"Generates diary-style summaries without code snippets.",
					},
				],
			});

			const confirmed = await this.confirmPersistentAutomation(
				scheduleTime,
				timezone,
			);
			if (!confirmed) {
				ui.callout({
					...
					subtitle:
						"Persistent unattended execution was cancelled before any scheduler changes were made.",
					lines: [
		...
		const nextStepMessage = this.getNextStepMessage(
			workingConfig,
			schedulerResult,
		);
		...
		ui.hero({
			...
			subtitle:
				"OpenMeta now has enough shape to scout, draft, and automate with intention instead of guesswork.",
			lines: [
		...
		ui.stats("Setup summary", [
			{ label: "GitHub", value: username, tone: "success" },
			{
				label: "Model",
				value: modelValue,
				hint: selectedProvider?.name,
				tone: "success",
			},
			{
				label: "Repo policy",
				value: targetRepoPath ? "CUSTOM" : "MANAGED",
				tone: "accent",
			},
			{
				label: "Automation",
		...
		const { pat } = await prompt<{ pat: string }>([
			{
				...
				mask: "*",
				validate: (input: string) =>
					input.trim().length > 0 || "PAT is required.",
			},
		]);
		return pat.trim();
	}
		...
		const { apiBaseUrl } = await prompt<{ apiBaseUrl: string }>([
			{
				...
				filter: (input: string) => input.trim(),
				validate: (input: string) =>
					input.trim().length > 0 || "API base URL is required.",
			},
		]);
	...
	private formatAutomationSummary(
		config: AppConfig,
	...
	private async confirmPersistentAutomation(
		scheduleTime: string,
		timezone: string,
	): Promise<boolean> {
		ui.callout({
		...
		ui.keyValues("Automation impact", [
			{
				...
				value: "Headless autonomous agent",
				tone: "warning",
			},
			{
				label: "Interactive review",
				value: "Skipped during scheduled runs",
				tone: "warning",
			},
			{ label: "Rollback", value: "openmeta automation disable", tone: "info" },
		]);

		const { acknowledgePersistence } = await prompt<{
		...
		}>([
			{
				...
				message:
					"Do you understand that this creates a long-running scheduled task on your machine?",
				default: false,
			},
		]);
