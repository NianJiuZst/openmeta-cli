export function parseGitHubRepoFullName(value: string): string {
	...
	try {
		...
		if (url.hostname.toLowerCase() !== 'github.com') {
			throw new Error(
				'Repository must be a GitHub repository, for example: https://github.com/vercel/next.js.',
			);
		}
...
export function resolveGitHubIssueTarget(issue: string, repo?: string): GitHubIssueTarget {
	...
	if (issueReference.repoFullName && repoFullName && issueReference.repoFullName !== repoFullName) {
		throw new Error(
			`Issue URL repository ${issueReference.repoFullName} does not match --repo ${repoFullName}.`,
		);
	}
	...
	if (!resolvedRepo) {
		throw new Error(
			'Issue number targets require --repo, for example: openmeta agent --repo owner/name --issue 123.',
		);
	}
