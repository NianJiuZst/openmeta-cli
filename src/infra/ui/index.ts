...
function buildCardText(
	...
	if (options.subtitle) {
		rows.push(
			...wrapLine(options.subtitle, width).map((line) => chalk.gray(line)),
		);
	}
	...
	if (options.lines && options.lines.length > 0) {
		if (variant === "callout") {
			rows.push(
				chalk.gray(renderRule(capabilities, tone, Math.min(24, width))),
			);
		}

		rows.push(
			...options.lines.flatMap((line) =>
				renderPrefixedLines(line, width, `${figures.bullet} `, "  "),
			),
		);
	}
...
function printCard(
	...
	process.stdout.write(
		`${boxen(content, {
			...
			borderStyle: capabilities.supportsUnicode
				? variant === "callout"
					? "double"
					: "round"
				: "single",
			...
			title:
				options.label && !genericCommandLabel(options.label)
...
function printHero(capabilities: UiCapabilities, options: CardOptions): void {
	...
	if (options.lines && options.lines.length > 0) {
		...
		const rendered = options.lines
			.flatMap((entry) =>
				renderPrefixedLines(entry, width - 2, `${bullet} `, "  "),
			)
			.map((line) =>
...
function printCelebration(
	...
	const rendered = (options.lines ?? [])
		.flatMap((entry) =>
			renderPrefixedLines(entry, width - 2, `${figures.pointerSmall} `, "  "),
		)
		.map((line) =>
...
function printSection(
	...
	printBlankLine();
	process.stdout.write(
		`${chalk.cyanBright.bold(`${figures.line} ${title}`)} ${chalk.gray(rule)}\n`,
	);
	if (subtitle) {
...
function printList(lines: string[], tone: Tone = "muted"): void {
	...
	for (const line of lines) {
		process.stdout.write(
			`${accent(figures.pointerSmall)} ${chalk.gray(line)}\n`,
		);
	}
}
...
function printKeyValues(
	...
	const labelWidth = Math.min(
		24,
		Math.max(...items.map((item) => item.label.length), 12),
	);
...
function printStats(
	...
	printSection(capabilities, title);
	const columns =
		capabilities.mode === "interactive-rich" && capabilities.width >= 96
			? 3
			: 2;
	const width = Math.max(44, Math.min(capabilities.width - 2, 106));
	...
	for (const [index, step] of steps.entries()) {
		...
		if (step.description) {
			for (const line of wrapLine(
				step.description,
				Math.max(36, capabilities.width - 14),
			)) {
				process.stdout.write(`      ${chalk.gray(line)}\n`);
			}
		}
...
function printTimeline(
	...
	for (const item of items) {
		...
		if (item.subtitle) {
			for (const line of wrapLine(
				item.subtitle,
				Math.max(36, capabilities.width - 12),
			)) {
				process.stdout.write(`      ${chalk.gray(line)}\n`);
			}
		}
...
function printRecordList(
	...
	for (const item of items) {
		...
		if (item.subtitle) {
			for (const line of wrapLine(
				item.subtitle,
				Math.max(36, capabilities.width - 10),
			)) {
				process.stdout.write(`      ${chalk.gray(line)}\n`);
			}
		}
...
function completionCopy(
	commandName: string,
): Pick<CardOptions, "title" | "subtitle" | "lines" | "tone"> {
	switch (commandName) {
		case "OpenMeta Agent":
			return {
				title: "The contribution arc resolved with a clean finish",
				subtitle:
					"The field, the repository, and the artifact trail now sit in one readable line of motion.",
				lines: [
		...
		case "OpenMeta Init":
			return {
				...
				subtitle:
					"Identity, model, profile, and automation posture now move as one system.",
				lines: [
		...
		case "OpenMeta Scout":
			return {
				...
				subtitle:
					"You now have a field worth choosing from, not just a pile worth scanning.",
				lines: [
		...
		case "OpenMeta Config":
			return {
				...
				subtitle:
					"Configuration output finished in a state you can inspect at a glance.",
				lines: [
		...
		case "OpenMeta Automation":
			return {
				...
				subtitle:
					"Scheduler state and local intent are no longer drifting apart.",
				lines: [
		...
		case "OpenMeta Inbox":
			return {
				...
				subtitle:
					"The most promising drafted opportunities are now easy to revisit without friction.",
				lines: [
		...
		case "OpenMeta PoW":
			return {
				...
				subtitle:
					"The record above can now be reviewed like a narrative instead of a dump.",
		...
		case "OpenMeta Doctor":
			return {
				...
				subtitle:
					"Runtime, configuration, storage paths, and automation policy are readable from one place.",
		...
		case "OpenMeta Runs":
			return {
				...
				subtitle:
					"Recent command execution is now traceable without digging through raw state.",
...
export const ui = {
	...
	commandCancelled(_commandName: string): void {
		...
		clackLines([
			chalk.gray(
				"Nothing was written, nothing was published, and the terminal remains in a safe state.",
			),
		]);
	},
	...
	commandFailed(_commandName: string, message: string): void {
		...
		clackLines([
			chalk.gray(
				"Inspect the blocking edge, fix it cleanly, then run again from a stable state.",
			),
		]);
	},
