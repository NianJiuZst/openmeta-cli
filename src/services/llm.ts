...
Repo Stars: ${i.repoStars}`,
	...
	async generateDailyDiary(
		issueAnalysis: string,
		userCodeSnippets: string,
	): Promise<string> {
		...
		const validationFailures =
			validationResults.length > 0
				? validationResults
						...
						.map((result) =>
							`${result.command} | exit=${result.exitCode ?? "n/a"}\n${result.output}`.trim(),
						)
						.join("\n\n---\n\n")
	...
	private async chat(
		...
		try {
			...
			throw new Error(
				"The LLM request failed. Please verify your provider, model, and API key.",
			);
		}
	}
	...
	private parsePatchDraft(
		content: string,
	): StructuredOutputResult<"patch_draft", PatchDraft> {
	...
	private extractJsonObject(content: string): string {
		...
		throw new Error(
			"LLM did not return a valid JSON object for the implementation draft.",
		);
	}
	...
	private describeValidationError(error: unknown): string {
		...
		if (status !== null) {
			const detail =
				LLM_VALIDATION_STATUS_HINTS[status] ??
				`The provider returned HTTP ${status} during validation.`;
	...
	private assertCustomValidationResponse(response: unknown): void {
		if (typeof response !== "object" || response === null) {
			throw new Error(
				"Custom provider validation response did not match the expected OpenAI-compatible format.",
			);
		}
		...
		if (!hasContent && !hasReasoningContent) {
			throw new Error(
				"Custom provider validation response did not include a usable assistant reply.",
			);
		}
	}
}
