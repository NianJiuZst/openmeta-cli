		...
		const next: RepoMemory = {
			...
			runStats: {
				...
				successfulValidationRuns:
					current.runStats.successfulValidationRuns + (validationSucceeded ? 1 : 0),
		...
		for (const path of changedFiles) {
			...
			nextSignals.set(path, {
				...
				successfulValidationCount:
					current.successfulValidationCount + (input.validationSucceeded ? 1 : 0),
	...
	private recordValidationFailures(
		...
		for (const result of validationResults.filter((result) => !result.passed)) {
			...
			nextSignals.set(result.command, {
				...
				lastSeenAt: now,
				sampleOutput:
					result.output.trim().replace(/\s+/g, ' ').slice(0, 180) || current.sampleOutput,
			});
		}
