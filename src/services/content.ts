...
export class ContentService {
  ...
  formatContributionDossier(
    ...
    const lines = [
      ...
      '',
      `- Detected Rule Files: ${workspace.contributionRules?.detectedFiles.join(', ') || 'none'}`,
      `- PR Template: ${workspace.contributionRules?.prTemplatePath || 'not detected'}`,
      `- Required Checklist Items: ${workspace.contributionRules?.requiredChecklistItems.join(' | ') || 'none'}`,
      `- PR Title Rules: ${workspace.contributionRules?.prTitleRules.join(' | ') || 'none'}`,
      `- Commit Message Rules: ${workspace.contributionRules?.commitMessageRules.join(' | ') || 'none'}`,
      `- Branch Naming Rules: ${workspace.contributionRules?.branchNamingRules.join(' | ') || 'none'}`,
      `- Validation Rules: ${workspace.contributionRules?.requiredValidationRules.join(' | ') || 'none'}`,
      `- Issue Linking Rules: ${workspace.contributionRules?.issueLinkingRules.join(' | ') || 'none'}`,
      `- Release Note Rules: ${workspace.contributionRules?.releaseNoteRules.join(' | ') || 'none'}`,
      `- Requires Prior Discussion: ${workspace.contributionRules?.requiresPriorDiscussion ? 'yes' : 'no'}`,
      `- Requires Passing Validation: ${workspace.contributionRules?.requiresPassingValidation ? 'yes' : 'no'}`,
      '',
