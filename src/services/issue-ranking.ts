...
export class IssueRankingService {
	...
	async loadTargetIssue(
		...
		if (!matched) {
			return opportunityService.rankIssues(
				this.buildLocalIssueMatches([issue], config.userProfile),
				config.scoring,
			);
		}
	...
	private scoreIssueForProfile(issue: GitHubIssue, userProfile: AppConfig['userProfile']): number {
		...
		if (
			/\b(blocked|needs info|needs information|duplicate|invalid|question|wontfix)\b/.test(labels)
		) {
			score -= 28;
		}
		...
		if (
			/\b(expected behavior|actual behavior|steps to reproduce|reproduction steps)\b/i.test(
				issue.body,
			)
		) {
			score += 8;
	...
	private estimateLocalWorkload(issue: GitHubIssue): string {
		...
		if (
			/\b(rewrite|migration|architecture|breaking change|refactor all|entire)\b/i.test(searchable)
		) {
