...
export class WorkspaceService {
	...
	async prepareRepositoryWorkspace(
		...
		const workspaceState = await this.prepareGitWorkspace(repoFullName, workspacePath, (git) =>
			this.createRepositoryAnalysisBranchName(git, repoFullName),
		);
		...
		return {
			...
			reviewReason:
				unsafeSkipped.length > 0
					? unsafeSkipped.map((file) => `${file.path}: ${file.reason}`).join('; ')
					: undefined,
		};
	}
	...
	private async createRepositoryAnalysisBranchName(
		git: SimpleGit,
		repoFullName: string,
	): Promise<string> {
	...
	private rankRepositoryAnalysisFiles(memory: RepoMemory, files: string[]): string[] {
		return [...files].sort(
			(left, right) =>
				this.scoreRepositoryAnalysisPath(right, memory) -
				this.scoreRepositoryAnalysisPath(left, memory),
		);
	}
	...
	private scoreRepositoryAnalysisPath(path: string, memory: RepoMemory): number {
		...
		const recentIssue = (memory.recentIssues ?? []).find((issue) =>
	...
	private detectTestCommands(workspacePath: string): TestCommand[] {
		...
		return commands.filter(
			(item, index, list) =>
				list.findIndex((candidate) => candidate.command === item.command) === index,
		);
	}
