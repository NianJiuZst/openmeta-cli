...
export class GitHubService {
	...
	async fetchTrendingIssues(
		...
		if (!options.refresh) {
			...
			if (cachedIssues) {
				logger.info(
					`Using cached GitHub issues (${cachedIssues.length}) to avoid unnecessary Search API calls.`,
				);
				return cachedIssues;
			}
		} else {
			logger.info(
				"Refreshing GitHub issue discovery and ignoring the local search cache.",
			);
		}
		...
		try {
			for (const labelGroup of FILTER_LABEL_GROUPS) {
				try {
					...
					logger.debug(
						`Fetched ${items.length} total results for "${labelGroup.join(" / ")}"`,
					);
					...
					logger.warn(
						`Issue search failed for labels "${labelGroup.join('" / "')}". ${failure.reason}`,
					);
				}
			}
			...
			if (
				...
				error.message.startsWith("GitHub issue discovery")
			) {
				throw error;
			}
			...
			throw new Error(
				"GitHub issue discovery failed. Please try again in a moment.",
			);
		}
	...
	private async fetchRepoMetadata(
		...
		try {
			...
			logger.debug(
				`Unable to fetch repository metadata for ${repoId.fullName}`,
				error,
			);
	...
	private describeSearchFailure(error: unknown): {
		reason: string;
		rateLimited: boolean;
	} {
		...
		if (err.status === 403) {
			return {
				reason:
					"GitHub Search API returned 403. This usually means rate limiting or secondary throttling.",
				rateLimited: true,
			};
		}
	...
	private async paginateSearchWithRetry(
		...
		while (attempt < MAX_PAGINATION_RETRIES) {
			try {
				...
				for await (const response of this.octokit.paginate.iterator(
					...
					const pageItems = (
						response as unknown as { data: { items: SearchIssueItem[] } }
					).data.items;
				...
				if (isRateLimit && attempt < MAX_PAGINATION_RETRIES) {
					...
					let delayMs =
						RATE_LIMIT_RETRY_BASE_DELAY_MS * 2 ** (attempt - 1);
		...
		throw new Error(
			`Pagination exhausted after ${MAX_PAGINATION_RETRIES} attempts`,
		);
	}
	...
	private getCachePath(): string {
		return join(
			ensureDirectory(join(getOpenMetaStateDir(), "cache")),
			"github-issues.json",
		);
	}
