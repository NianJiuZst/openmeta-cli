...
export class GitHubService {
	...
	async fetchTrendingIssues(options: IssueDiscoveryOptions = {}): Promise<GitHubIssue[]> {
		...
		const repoFullName = options.repoFullName
			? parseGitHubRepoFullName(options.repoFullName)
			: undefined;
		...
		try {
			if (repoFullName) {
				try {
					...
					const items = await this.paginateSearchWithRetry(
						searchQuery,
						[repoFullName],
						options.onStatus,
					);
	...
	async fetchIssue(repoFullName: string, issueNumber: number): Promise<GitHubIssue> {
		...
		const repoId = this.parseRepositoryUrl(
			item.repository_url || `https://api.github.com/repos/${normalizedRepo}`,
		);
		...
		while (attempt < MAX_PAGINATION_RETRIES) {
			try {
				while (pagesFetched < MAX_SEARCH_PAGES) {
					...
					pagesFetched++;
					const hasMorePages =
						pageItems.length > 0 && totalCount > pagesFetched * SEARCH_RESULTS_PER_PAGE;
				...
				if (isRateLimit) {
					if (attempt < MAX_PAGINATION_RETRIES) {
						...
						let delayMs =
							RATE_LIMIT_RETRY_FALLBACK_DELAY_MS +
							RATE_LIMIT_RETRY_FALLBACK_DELAY_MS * (attempt - 1);
						...
						logger.debug(
							`Pagination retries exhausted. Yielding ${items.length} items collected so far.`,
						);
						onStatus?.(
