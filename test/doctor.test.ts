...
describe("DoctorOrchestrator", () => {
	...
	test("reports a ready local surface when required config and directories are present", async () => {
		...
		expect(
			report.checks.find((check) => check.id === "github-config")?.status,
		).toBe("pass");
		expect(
			report.checks.find((check) => check.id === "llm-config")?.status,
		).toBe("pass");
	});
	...
	test("fails when a configured artifact repository path does not exist", async () => {
		...
		expect(report.ready).toBe(false);
		expect(
			report.checks.find((check) => check.id === "target-repo")?.status,
		).toBe("fail");
	});
});
