...
interface GitHubServiceInternals {
	octokit: {
		rest: {
			search: {
				issuesAndPullRequests: () => Promise<{
					data: { total_count: number; items: unknown[] };
				}>;
			};
		};
		paginate: {
	...
	extractLabelNames(item: {
		labels: Array<string | { name?: string | null }>;
	}): string[];
	describeSearchFailure(error: unknown): {
		reason: string;
		rateLimited: boolean;
	};
...
describe("GitHubService internals", () => {
	...
	test("builds GitHub search queries with label groups and common filters", () => {
		...
		expect(query).toContain(
			'label:"good first issue" OR label:"good-first-issue"',
		);
	...
	test("filters out pull requests, locked issues, and assigned issues", () => {
		...
		expect(service.shouldIncludeIssue({ assignee: { login: "owner" } })).toBe(
			false,
		);
		expect(
			service.shouldIncludeIssue({ assignees: [{ login: "owner" }] }),
		).toBe(false);
		expect(service.shouldIncludeIssue({ locked: false, assignees: [] })).toBe(
			true,
		);
	});
	...
	test("parses repository URLs and rejects malformed URLs", () => {
		...
		expect(
			service.parseRepositoryUrl("https://api.github.com/repos/acme/demo"),
		).toEqual({
			owner: "acme",
			repo: "demo",
			fullName: "acme/demo",
		...
		expect(() =>
			service.parseRepositoryUrl("https://api.github.com/repos/acme"),
	...
	test("persists and reloads fresh issue cache entries", () => {
		...
		expect(readFileSync(cachePath, "utf-8")).toContain(
