...
describe('parseGitHubIssueReference', () => {
	...
	test('rejects pull request URLs and invalid numbers', () => {
		...
		expect(() =>
			parseGitHubIssueReference('https://github.com/Wei-Shaw/sub2api/pull/3014'),
		).toThrow('Issue must be a GitHub issue URL or positive issue number');
	});
});
...
describe('resolveGitHubIssueTarget', () => {
	...
	test('rejects repository mismatches between --repo and --issue URL', () => {
		expect(() =>
			resolveGitHubIssueTarget('https://github.com/Wei-Shaw/sub2api/issues/3014', 'vercel/next.js'),
