...
interface AgentOrchestratorInternals {
	...
	countValidationStates(results: TestResult[]): {
		passed: number;
		failed: number;
		unavailable: number;
	};
	...
	ensureTargetRepo(
		config: AppConfig,
	): Promise<{ path: string; owner: string; repo: string; defaultBranch: string }>;
...
describe('AgentOrchestrator support behavior', () => {
	...
	test('promptForIssue exposes numeric fallback validation bounds', async () => {
		...
		spyOn(infra, 'selectPrompt').mockRejectedValue(new Error('tty unavailable'));
		const promptSpy = spyOn(infra, 'prompt').mockImplementation(
			async <T extends object>(questions: unknown) => {
	...
	test('promptForIssue rethrows user cancellation from the interactive selector', async () => {
		...
		await expect(orchestrator.promptForIssue(issues)).rejects.toBeInstanceOf(
			infra.UserCancelledError,
		);
	});
	...
	test('publishArtifactsIfNeeded returns preview data in dry-run mode and skips publish when confirmations fail', async () => {
		...
		spyOn(
			orchestrator as object as { ensureTargetRepo: () => Promise<unknown> },
			'ensureTargetRepo',
		).mockResolvedValue({
			path: targetRepoPath,
			owner: 'octocat',
	...
	test('publishArtifactsIfNeeded throws on git setup failures and reports success after publishing', async () => {
		...
		await expect(
			orchestrator.publishArtifactsIfNeeded(createPublishInput({ config })),
		).rejects.toThrow(`Failed to initialize the target repository at ${targetRepoPath}.`);
	...
	test('showInbox and showProofOfWork render empty and populated states', async () => {
		...
		spyOn(inboxService, 'load')
			...
			.mockReturnValueOnce({
				items: [
					createInboxItem({ generatedAt: '2026-06-03T00:00:00.000Z' }) as ContributionInboxItem,
				],
			});
		spyOn(proofOfWorkService, 'load')
			...
			.mockReturnValueOnce({
				records: [
					createProofRecord({ generatedAt: '2026-06-03T00:00:00.000Z' }) as ProofOfWorkRecord,
				],
			});
	...
	test('scout renders an empty state when no issues meet the thresholds', async () => {
		...
		spyOn(
			AgentOrchestrator.prototype as unknown as {
				validateConfig: AgentOrchestratorInternals['validateConfig'];
			},
			'validateConfig',
		).mockResolvedValue(undefined as never);
		spyOn(
			AgentOrchestrator.prototype as unknown as {
				initializeClients: AgentOrchestratorInternals['initializeClients'];
			},
			'initializeClients',
		).mockResolvedValue(undefined as never);
	...
	test('collects confirmations for headless and publish prompts', async () => {
		...
		await expect(
			orchestrator.promptForContributionPrConfirmation(createRankedIssue()),
		).resolves.toBe(true);
	...
	test('ensureManagedRemoteRepo reads existing remotes, creates missing ones, and requires octokit initialization', async () => {
		...
		await expect(
			orchestrator.ensureManagedRemoteRepo('octocat', 'openmeta-daily'),
		).resolves.toEqual({
	...
	test('getGitHubRepositoryInfo requires octokit and returns repository metadata when initialized', async () => {
		...
		await expect(
			orchestrator.getGitHubRepositoryInfo('octocat', 'openmeta-daily'),
		).resolves.toEqual(
			expect.objectContaining({
				default_branch: 'main',
			}),
	...
	test('detectLocalDefaultBranch prefers origin HEAD, common branch names, then the current branch', async () => {
		...
		await expect(
			orchestrator.detectLocalDefaultBranch({
				...
				branch: async () => ({
					all: ['openmeta-artifacts', 'main'],
					current: 'openmeta-artifacts',
				}),
			}),
		).resolves.toBe('main');
	...
	test('ensureTargetRepo uses configured targets and managed home repositories', async () => {
		...
		spyOn(
			orchestrator as object as {
				getGitHubRepositoryInfo: () => Promise<{ default_branch: string }>;
			},
